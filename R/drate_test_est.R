...
# drate_values <- c(.0014, .0018, .0015, .0020, .0030)


drate_main <- c(.0018# , .0020, .0022
                )
drate_cas <- c(# .0010, .0012,
               .0014)

# drate_mat <- data.frame(drate_main = rep(drate_main, each = length(drate_main)),
#                         drate_cas = rep(drate_cas, length(drate_cas))) %>%
#   dplyr::mutate(drate_char = paste(as.character(drate_main), as.character(drate_cas), sep = "_")) %>%
#   dplyr::mutate(drate_char = stringr::str_replace_all(drate_char, "\\.", ""))


drate_mat <- data.frame(drate_main = drate_main,
                        drate_cas = drate_cas,
                        venue_target_main = target_extract(term = "fuzzynodematch.venues_all.TRUE", model = "main") + c(-50),
                        venue_target_cas = target_extract(term = "fuzzynodematch.venues_all.TRUE", model = "casual") + c(-150),
                        venue_target_onetime = target_extract(term = "fuzzynodematch.venues_all.TRUE", model = "one.time") + c(-9)) %>%
  # dplyr::mutate(drate_char = paste(as.character(drate_main), as.character(drate_cas), sep = "_")) %>%
  # dplyr::mutate(drate_char = stringr::str_replace_all(drate_char, "\\.", ""))
  dplyr::mutate(drate_char = "best_guess")


for (m in 1:nrow(drate_mat)) {
...
netstats <- list(
  ...
  main = list(
    ...
    fuzzynodematch_venues.all = drate_mat[m, "venue_target_main"],
    ...
    # dissolution model
    dissolution = dissolution_coefs(~offset(edges), duration = 87, d.rate = drate_mat[m, "drate_main"]),
    diss.homog = dissolution_coefs(dissolution = ~offset(edges),
                                   duration = dur_coefs$main$durs.main.homog$mean.dur.adj,
                                   d.rate = drate_mat[m, "drate_main"]),
  ...
  casl = list(
    ...
    fuzzynodematch_venues.all = drate_mat[m, "venue_target_cas"],
    ...
    # dissolution model
    dissolution = dissolution_coefs(~offset(edges), duration = 57, d.rate = drate_mat[m, "drate_cas"]),
    diss.homog = dissolution_coefs(dissolution = ~offset(edges),
                                   duration = dur_coefs$casl$durs.casl.homog$mean.dur.adj,
                                   d.rate = drate_mat[m, "drate_cas"]),
    diss.byage = dissolution_coefs(dissolution = ~offset(edges) +
                                     offset(nodematch("age.grp", diff = TRUE)),
  ...
  inst = list(
    ...
    fuzzynodematch_venues.all = drate_mat[m, "venue_target_onetime"],
...
saveRDS(netstats, paste0(est_dir, "netstats-level-", context, "_", drate_mat[m, "drate_char"], "_targetminus", ".rds"))
...
saveRDS(out, paste0(est_dir, "basic_netest-level-allvenues", context, "_", drate_mat[m, "drate_char"], "_alt", ".rds"))
...
out <- list(fit_main = fit_main, fit_casl = fit_casl, fit_inst = fit_inst)
saveRDS(out, paste0(est_dir, "venues_apps_netest-level-allvenues", context, "_", drate_mat[m, "drate_char"], "_targetminus", ".rds"))
...
out <- list(fit_main = fit_main, fit_casl = fit_casl, fit_inst = fit_inst)
saveRDS(out, paste0(est_dir, "apps_only_netest-level-allvenues", context, "_", drate_mat[m, "drate_char"], "_targetminus", ".rds"))
...
out <- list(fit_main = fit_main, fit_casl = fit_casl, fit_inst = fit_inst)
saveRDS(out, paste0(est_dir, "venue_only_netest-level-allvenues", context, "_", drate_mat[m, "drate_char"], "_targetminus", ".rds"))
